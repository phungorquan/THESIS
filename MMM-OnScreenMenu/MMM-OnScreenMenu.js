        ...
        if (typeof modules !== "undefined") {
            modules.forEach(k => {
                if (action.indexOf("Hide") > -1 ||
                    ...
                    // Xiu add to toggle MMM-OnOffModules
                    // This will ignore my module
                    if(this.config.menuItems[action].name === "MMM-OnOffModules");
                    else k.hide(0, { lockString: "osm" });
                    ...
                    // This will ignore my module
                    if(this.config.menuItems[action].name === "MMM-OnOffModules");
                    else k.show(1500, { lockString: "osm" });
                }

                // Xiu add to toggle MMM-OnOffModules
                // Send notification to my module to process hide or show
                if (this.config.menuItems[action].name === "MMM-OnOffModules")
                {
                    this.sendNotification("TOGGLE_ONOFFMODULES", "DUMMY_DATA");
                }
